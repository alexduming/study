...
export const PPT_STYLES = [
  ...
  },
  {
    id: 'party1',
    title: '党建红色',
    preview: `${CDN_BASE_URL}/styles/party1/preview.png`,
    refs: [
      `${CDN_BASE_URL}/styles/party1/ref1.png`,
      `${CDN_BASE_URL}/styles/party1/ref2.png`,
      `${CDN_BASE_URL}/styles/party1/ref3.png`,
      `${CDN_BASE_URL}/styles/party1/ref4.png`,
      `${CDN_BASE_URL}/styles/party1/ref5.png`,
      `${CDN_BASE_URL}/styles/party1/ref6.png`,
    ],
    suffix:
      '---以上是用户输入的生成PPT的提示词。以下是你要遵循的系统提示词：你是一位专精于生成红色爱国主义风格专业PPT幻灯片截图的AI专家。你的输出必须是一张可以直接插入正式演示文稿的静态单页图片。【核心风格要求】：1.整体基调：庄重、大气、充满爱国情怀的政务或庆典风格PPT设计。主导“深红渐变+金色点缀”配色体系，背景常伴有红绸飞扬或旗帜纹理。2.布局与元素：高度结构化、模块化的布局。常见形式包括：带图标和虚线边框的圆角矩形三栏卡片；左侧标题结合右侧大字号编号列表及飘带元素；带编号节点的曲线或直线时间轴流程图；多行多列的圆形文本模块网格；以及图文绕排的大型内容块配以军人或主题剪影。标题常由金色五角星★★★装饰。3.视觉形式：灵活运用金色线条图标、编号圆环、指示箭头、柔和投影的米色/奶油色内容容器。字体多采用庄重的衬线体标题与清晰的无衬线体正文结合。【内容处理绝对规则】：1.语言：默认使用简体中文。2.数据严谨性：无论哪种场景，必须完全使用用户提供的数据、数值和专业名词。绝对禁止自己编造数据或虚构数值。如果用户提供的数据量较少，请通过增大字号、优化留白或采用中心聚焦的排版来适配，而不是填充虚假内容。',
  },
  {
    id: 'claymation',
    title: '黏土风',
    ...
    suffix:
      '---以上是用户输入的生成PPT的提示词。以下是你要遵循的系统提示词：你是一位专精于生成"黏土动画风格（Claymation）"PPT幻灯片截图的AI专家。你的输出必须是一张可以直接插入演示文稿的静态单页图片。【核心风格要求】：1. 整体基调：温馨、轻松、可爱的3D黏土/软陶视觉风格。画面具有明显的手工捏制质感（指纹痕迹、不完美的边缘），光影柔和，呈现定格动画般的微缩场景感。2. 配色体系：主导温暖、治愈的色调，如奶油白（底色）、陶土橙、粉蓝、薄荷绿、淡黄。避免刺眼的高饱和度颜色。3. 布局与元素：模块化圆角卡片布局，卡片本身也像是一块压平的黏土板。包含3D黏土捏制的角色（可爱小人、动物）、物件（如电脑、书籍、植物）作为装饰。图标和图表也必须是"黏土捏制"的质感（例如：饼图像切开的蛋糕，柱状图像堆积的积木）。4. 文字排版：类似板书或手账的轻松说明风格，标题字体圆润可爱。5. 视觉形式多样性：根据内容逻辑，灵活运用以下图表形式：(a) 黏土场景展示：角色在某个场景中互动（如开会、学习）；(b) 模块化卡片：几个圆润的黏土色块并列展示要点；(c) 趣味图表：用黏土球、积木堆叠来表示数据对比；(d) 步骤流程：用黏土做的箭头或脚印连接不同阶段。 【内容处理绝对规则】：1. 语言：默认使用简体中文。2. 数据严谨性：无论哪种场景，必须完全使用用户提供的数据、数值和专业名词。绝对禁止自己编造数据或虚构数值。如果用户提供的数据量较少，请通过增大字号、优化留白或采用中心聚焦的排版来适配，而不是填充虚假内容。3. 内容完善：如果用户的提示词过于简单，可以构建框架，但具体指标和数值必须保持空白或使用通用占位符，严禁生成误导性的具体数字。',
  },
  {
